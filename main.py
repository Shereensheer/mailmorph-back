...
# # ------------------- Config -------------------
# os.environ["OAUTHLIB_INSECURE_TRANSPORT"] = "1"  # localhost dev
...
# # ------------------- App Init -------------------
# app = FastAPI(title="MailMorph API with Leads & AI Cold Email Agent")
# app.add_middleware(
#     CORSMiddleware,
...
#             "id": str(uuid4()),                # ✅ unique id for delete
#             "to": req.to,
#             "subject": req.subject,
#             "body": req.body,
#             "threadId": thread_id,
...
# # ------------------- Send a Reply (same thread) -------------------
# @app.post("/reply")
# def api_reply(req: ReplyReq):
#     """
#     Bheje gaye thread me reply karta hai.
#     Subject auto "Re: <original subject>" (agar mil jaye).
#     Gmail me thread join karne ke liye 'threadId' set karte hain.
#     """
#     try:
#         service = get_gmail_service()
#         if not service:
...
#         # Try to fetch original subject from the thread
#         try:
...
#         message["threadId"] = req.threadId  # join existing thread

#         sent = service.users().messages().send(userId="me", body=message).execute()

#         # Save this outgoing reply locally so UI me turant dikh jaye
...
# def api_sent():
#     try:
#         items = email_storage.load_emails()
#         return {"items": items}
#     except Exception as e:
#         raise HTTPException(status_code=500, detail=str(e))
...
# def get_latest_replies(thread_ids: str = ""):
#     ids = thread_ids.split(",") if thread_ids else []
#     return {"items": email_storage.load_new_replies(ids)}

# # ------------------- Sync INCOMING replies from Gmail -------------------
# @app.post("/sync-replies")
# def sync_replies():
#     """
#     Gmail se threads pull karke incoming replies ko local store me save karta hai.
...
#         sent_items = email_storage.load_emails()
#         existing = email_storage.load_replies()
#         existing_ids = {r.get("msgId") for r in existing if r.get("msgId")}

#         new_count = 0
#         for it in sent_items:
#             t_id = it.get("threadId")
#             if not t_id:
#                 continue
#             thread = service.users().threads().get(userId="me", id=t_id).execute()
#             messages = thread.get("messages", [])
#             if not messages:
#                 continue

#             # Skip the first original (usually the sent), iterate all to catch any not saved yet
#             for msg in messages:
#                 msg_id = msg.get("id")
...
#                 frm = _get_header(headers, "From") or ""
#                 subject = _get_header(headers, "Subject") or ""
#                 # Only treat as incoming reply if From is not me
#                 if my_email and my_email in frm.lower():
#                     continue

#                 # Extract body (handles simple text/plain)
#                 body_data = ""
#                 if payload.get("body", {}).get("data"):
#                     body_data = base64.urlsafe_b64decode(payload["body"]["data"]).decode(errors="ignore")
#                 else:
#                     # Try parts
#                     for part in payload.get("parts", []) or []:
#                         if part.get("mimeType") == "text/plain" and part.get("body", {}).get("data"):
#                             body_data = base64.urlsafe_b64decode(part["body"]["data"]).decode(errors="ignore")
#                             break
...
#     return {"ok": True, "message": f"Lead {email_to_delete} deleted successfully"}
...
# from services.gmail_auth import get_gmail_service
# from services.email_storage import save_reply, load_replies

# router = APIRouter()

# @router.get("/fetch-replies")
# def fetch_replies():
#     try:
#         service = get_gmail_service()
...
# from services.reply import fetch_replies
...
# from services import email_storage
# from services.ai_writer import generate_email
# from services.reply import fetch_replies   # ✅ added for replies
...
# app.add_middleware(
#     CORSMiddleware,
#     allow_origins=["*"],  # In production, restrict frontend URL
#     allow_credentials=True,
#     allow_methods=["*"],
#     allow_headers=["*"],
# )
...
#     save_leads(renumber_leads(new_leads))
...
# # ------------------- App Init -------------------
# app = FastAPI(title="MailMorph API with Leads, Users & AI Cold Email Agent")
# app.add_middleware(
#     CORSMiddleware,
#     allow_origins=["*"],  # In production, restrict frontend URL
#     allow_credentials=True,
...
# def save_users(users: list): save_pickle(USERS_FILE, users)
# def load_users(): return load_pickle(USERS_FILE)
...
#             # ✅ Agar image bheji gayi hai to save kar lo
#             if profilePic:
#                 os.makedirs("uploads", exist_ok=True)
#                 file_location = f"uploads/{u['id']}_{profilePic.filename}"
#                 with open(file_location, "wb") as f:
#                     f.write(await profilePic.read())
#                 u["profilePic"] = f"/{file_location}"
